...
"""Crawls specified sites for new articles and processes them into the database."""
...
async def save_article_to_relevancy(conn: aiomysql.Connection,
                                    cur: aiomysql.Cursor,
                                    article_info: ArticleInfo,
                                    article_text: str):
    """Saves an article to CrawlerToRelevancy."""
    while True:
        try:
...
def fix_escaped_unicode(i) -> str:
    """Fixes weirdly escaped Unicode. Mostly applies to Arabic text."""
    b = []
    e = 0
...
async def fetch_full_article(ctx: CrawlerContext, url: str) -> newspaper.Article:
    """Fetches the full article (for RSS) using newspaper."""
...
async def article_seen(conn: aiomysql.Connection, cur: aiomysql.Cursor, art: ArticleInfo) -> bool:
    """Checks if an article has previously been encountered by the crawler."""
...
async def feeder(ctx: CrawlerContext, feed: Feed):
    """Fetches for and manages a feed."""
    while True:
        ...
        match feed.form:
            case "rss" | "erss" | "rdo":
                async with ctx.session.get(feed.url, headers={
                    ...
                    articles = feedparser.parse(str(await resp.read()),
                                                response_headers=headers)["entries"]
                for article in articles:
                    ...
                    elif feed.form in ("rss", "erss"):
                        article_text = "Summary: " \
                                       + article["description"] \
                                       + "\nText: " \
                                       + (await fetch_full_article(ctx, article["link"])).text
                    elif feed.form == "rdo":
                        article_text = "Text: " + article["description"]
                    logger.debug(f"Read {title} from {feed.publisher}")
                    await save_article_to_relevancy(conn,
                                                    cur,
                                                    article_info,
                                                    fix_escaped_unicode(article_text))
            case "cnn" | "maan" | "hespress" | "n3k":
                # TODO: fork newspaper3k/4k, use async requests only
                # TODO: someone clean up this hot garbage code please
                if feed.form == "cnn":
                    articles = [article.url
                                for article in newspaper.build("https://cnn.com").articles
                                if (
                    ...
                    articles = [article.url
                                for article in newspaper.build("https://www.maannews.net").articles
                                if article.url.endswith(".html")]
                elif feed.form == "hespress":
                    articles = [article.url
                                for article in newspaper.build("https://www.hespress.com").articles
                                if (
...
async def main():
    """Launches the crawler and initializes global structures."""
    # Initialization steps
    os.chdir(os.path.dirname(os.path.abspath(__file__)))
    with open("../../secrets/db.json", "r", encoding="utf-8") as _f:
        creds = orjson.loads(_f.read())
    pool = await aiomysql.create_pool(autocommit=False, maxsize=10, **creds)
    with open("feeds.json", "r", encoding="utf-8") as _f:
        feeds = [Feed(i["url"], i["publisher"], i["format"])
                 for i in orjson.loads(_f.read())]
    ...
    logger.add(sys.stdout,
               ...
               format="<green>{time: YYYY-MM-DD HH:mm:ss.SSS}</green> "
                      "<level>{level}</level> <b>(crawler)</b> {message}")
    loguih.setup()
